...
export default function DealsOfTheDay() {
  ...
  useEffect(() => {
    fetch('https://fakestoreapi.com/products')
      .then(res => res.json())
      .then(data => {
        // Simulate deals: pick 6 random discounted products
        const shuffled = data.sort(() => 0.5 - Math.random());
        const selectedDeals = shuffled.slice(0, 6).map(product => ({
          ...product,
          oldPrice: (product.price * 1.2).toFixed(2) // Simulated 20% off
        }));
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {deals.map((product) => (
          <div key={product.id} className="relative">
            <span className="absolute top-2 left-2 bg-red-500 text-white text-xs px-2 py-1 rounded z-10">20% OFF</span>
